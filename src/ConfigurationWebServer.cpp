                ...
                <div class="flex flex-col gap-2">
                    <label class="flex flex-col sm:flex-row gap-2 flex-1">
                        ...
                        <button
                            type="button"
                            id="lookup-btn"
                            class="border border-green-500 px-4 py-2 sm:py-0 whitespace-nowrap">
                            Find
                        </button>
                    </label>
                    <div id="city-result" class="text-xs opacity-80"></div>
                </div>
            ...
            document.getElementById('lookup-btn').addEventListener('click', async function() {
                ...
                try {
                    ...
                    if (!data.results || data.results.length === 0) {
                        resultDiv.textContent = 'No match found for "' + city + '".';
                        return;
                    }

                    const place = data.results[0];
                    document.querySelector('input[name="latitude"]').value = place.latitude.toFixed(6);
                    document.querySelector('input[name="longitude"]').value = place.longitude.toFixed(6);
                    document.getElementById('city').value = place.name;   

                    const region = place.admin1 ? ', ' + place.admin1 : '';
                    resultDiv.textContent = 'Found: ' + place.name + region + ', ' + place.country +
                        ' (' + place.latitude.toFixed(4) + ', ' + place.longitude.toFixed(4) + ')';
                } catch (err) {
                    resultDiv.textContent = 'Lookup failed — check your internet connection.';
                }
            });
        </script>
    </body>
...
void ConfigurationWebServer::Initialise() {
    ...
    server.on("/", HTTP_GET, [&](AsyncWebServerRequest* request) {
        ...
        AsyncWebServerResponse* response = request->beginResponse(
            ...
            [latitude, longitude, radius, city, openskyClientId, openskySecret, scanlineEnabled, infoTextEnabled, triangleEnabled]
            (const String& var) -> String {
                if (var == "LATITUDE")       return latitude;
                if (var == "LONGITUDE")      return longitude;
                if (var == "RADIUS")         return radius;
                if (var == "CITY")           return city;
